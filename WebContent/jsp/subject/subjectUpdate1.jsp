...
<body>
	<s:form id = "form" action = "updateSubject" onsubmit = "return formCheck()" theme = "simple">
			...
			<tr>
				...
				<td><select name="class_num1" id="class_num1">
						...
						<s:iterator value="selLists" status="stat">
							...
							<s:else>
								<option><s:property value="selList[#stat]"/></option>
							</s:else>
				...
				<td rowspan="2"><input type="text" name="subjectname" id="subjectname"
					value="<s:property value="subj.subjectname"/>"></td>
				...
				<td rowspan="2">시작<select id="startyear" name="startyear">
						<s:iterator var="i" begin="2014" end="2000" step="-1">
							 ...
							 종료<select id="endyear" name="endyear">
						<s:iterator var="i" begin="2014" end="2000" step="-1">
									<s:if test="#i==endyear">
										<option selected="selected"><s:property value ="#i"/>
									</s:if>
				...
				<td><textarea rows="" cols="" name="content"><s:property value="subj.content"/></textarea></td>
				<td align="center"><input type="button" value="수정완료" onclick="javascript:updateSubject()"></a></td>
				
			</tr>
			<tr>
			
				<td><select name="class_num2" id="class_num2">



						<s:iterator value="selLists" status="stat">

							<s:if test="subj1.class_num==selLists[#stat.index]">
								<option selected="selected"><s:property /></option>
							</s:if>
							<s:else>
				...
				<td><select name="teacher2" id="teacher2">
						<s:iterator value="teacherLists" status="stat">
							<s:if test="key1==teacherLists[#stat.index].name">
								<option selected="selected" value='<s:property value="id"/>'><s:property value="name" /></option>
							</s:if>
							<s:else>
								<option value='<s:property value="id"/>'><s:property value="name" /></option>
