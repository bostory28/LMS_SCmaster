...
<head>
  ...
  /*   function getfalgs(){
					...
					$.each(json.attendanceList, function(index,attendance){
						if(student.id==attendance.id){
							...
							if(attendance.attendance_check==1){
								...
								// 여기서는 현재 관리자 이므로 지각, 결석 모든 란에 링크를 걸어줌 - 완료
								// 인터셉터를 통해 페이지를 처리하자... 학생이 보는 출석부는 다른 페이지로 구성한다. 
								// attendanceList_student
								check_figure = '<a class="iframe" href="javascript:statementRead('+statement_key+');">▲</a>';
							}else{
								check_figure = '<a class="iframe" href="javascript:statementRead('+statement_key+');">x</a>';
							}
	...
	function statementRead(statement_key){
		...
		$.ajax({
			...
			success : function (json) {
				...
				if(json.statementVo==null){
					alert("아직 작성되지 않았습니다. 학생을 갈구십시오!");
					return;
				}
				var fullContent = "";
				fullContent += "<table border='1'>";
				fullContent += 		"<tr>";
				fullContent += 			"<th width='120px'>소 속</th><td width='500px'><input style='resize:none; width:100%;border:none;' type='text' class='statement'  readonly='readonly'  value="+json.statementVo.affiliation+"></td>";
				fullContent += 		"</tr>";
				fullContent += 		"<tr>";
				fullContent += 			"<th>성 명</th><td><input style='resize:none; width:100%;border:none;' type='text' class='statement'  readonly='readonly'  value="+json.statementVo.name+"></td>";
				fullContent += 		"</tr>";
				fullContent += 		"<tr>";
				fullContent += 			"<th>사고 내용</th><td><input style='resize:none; width:100%;border:none;' type='text' class='statement'  readonly='readonly'  value="+json.statementVo.content+"></td>";
				fullContent += 		"</tr>";
				fullContent += 		"<tr>";
				fullContent += 			"<th height='200px'>사고 사유</th><td><textarea style='resize:none; width:100%;border:none;' rows='10'  readonly='readonly'  class='statement'>"+json.statementVo.reason+"</textarea></td>";
				fullContent += 		"</tr>";
				fullContent += 		"<tr>";
				fullContent += 			"<th height='200px'>처리 및 조치방안</th><td><textarea style='resize:none; width:100%;border:none;' rows='10'  readonly='readonly'  class='statement'>"+json.statementVo.afterplan+"</textarea></td>";
				fullContent += 		"</tr>";
				fullContent += 		"<tr>";
				fullContent += 			"<td colspan='2'>작성자 본인은 처리 및 조치가 이행되지 않을 경우 모든 행정적 불이익을 감수함<br>";
				fullContent +=			"2014년   "+(new Date().getMonth()+1)+" 월  "+new Date().getDate()+" 일<br>";
				fullContent +=			"성명   :"  +json.statementVo.name+"    (서명)</td>";
				fullContent += 		"</tr>";
				fullContent += "</table>";
				// 선생님, 관리자는 작성된 사유서를 보기만 하기때문에 등록버튼 필요없음!	
				//	fullContent += "<input type='button' value='등록'/>";
