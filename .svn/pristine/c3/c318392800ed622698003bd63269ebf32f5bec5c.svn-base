   ...
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    $(document).ready(function() {
		...
		$('#inserttestbtn').click(function() {
			...
			window.open('insertMonthForm.action', 'insertMonthForm', 'width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		});
		//검색기능
    ...
    function editForm(testname, flag) {
		 ...
		 ml=(cw-sw)/2;        //가운데 띄우기위한 창의 x위치
		 mt=(ch-sh)/2;         //가운데 띄우기위한 창의 y위치
    	window.open('editMonthForm.action?&flag='+flag+ '&testname='+testname, 'editSubjectForm', 'width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
    }
    
    function search() {
    	var flag = $('#flag_search').val();
    ...
    function output(json) {
    	...
    	for(var i = 0; i < json.list_test.length; i++) {
    		if(s == 0 || json.list_test[i].teachername != c) {
	   		...
	   		if(json.list_test[i].avg_score == 0) {
	    		str += "<tr><td>"+ json.list_test[i].flag +"</td><td>"+ json.list_test[i].testname +"</td><td>"+ json.list_test[i].startdate +"</td><td>"+ json.list_test[i].enddate +"</td><td>"+ json.list_test[i].limittime +"</td><td>"+ json.list_test[i].avg_score.toFixed(1) +"</td>";
	    		if(session == 'admin' || session == json.list_test[i].teacherid) {
	   			...
	   			if(session == 'admin' || session == json.list_test[i].teacherid) {
	    			str += "<tr><td>"+ json.list_test[i].flag +"</td><td>"+ json.list_test[i].testname +"</td><td>"+ json.list_test[i].startdate +"</td><td>"+ json.list_test[i].enddate +"</td><td>"+ json.list_test[i].limittime +"</td><td>"+ json.list_test[i].avg_score.toFixed(1) +"</td><td><input type='button' flag='"+json.list_test[i].flag+"' class_num='"+json.list_test[i].class_num+"' subjectid='"+json.list_test[i].subjectid+"' testid='"+json.list_test[i].testid+"' testname='"+json.list_test[i].testname +"' class='editgradebtn' value='성적수정'/></td></tr>";
	   			}
	   		}
    	}
    	str += '</table><br>';
    	...
    	$('.gradebtn').click(function() {
	    	...
	    	cw=screen.availWidth;     //화면 넓이
			 ...
			 ml=(cw-sw)/2;        //가운데 띄우기위한 창의 x위치
			 mt=(ch-sh)/2;         //가운데 띄우기위한 창의 y위치
			window.open('insertMonthGradeForm.action?teasubteavo.testname='+testname +'&teasubteavo.flag='+flag+ '&teasubteavo.class_num='+class_num+ '&teasubteavo.subjectid='+subjectid+'&teasubteavo.testid='+testid+'&flag='+flag_search +'&class_num='+class_num_search, 'insertGradeForm', 'width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		});
    	...
    	$('.editgradebtn').click(function() {
	    	...
	    	var class_num_search = $('#class_num').val();
			 ...
			 ml=(cw-sw)/2;        //가운데 띄우기위한 창의 x위치
			 mt=(ch-sh)/2;         //가운데 띄우기위한 창의 y위치
			window.open('editMonthGradeForm.action?teasubteavo.testname='+ testname +'&teasubteavo.flag='+flag+ '&teasubteavo.class_num='+class_num+ '&teasubteavo.subjectid='+subjectid+'&teasubteavo.testid='+testid+'&flag='+flag_search +'&class_num='+class_num_search, 'editMonthGradeForm', 'width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		});
    }
    </script>
</head>
       		...
       		<div>
				...
				<table align="center" border="0" width="100%">
       				<tr>
       					...
       					<td rowspan="2">
       						&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
       						&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<b style="font-size: 30px">[월말시험리스트]</b>
       					</td>
       				</tr>
       				<tr>
       					<td>
       			...
       			<s:if test="#session.userSign.type == 0">
	       			<input type="button" value="일본어월말시험등록" class="btn btn-primary" id="inserttestbtn"><br>
	       		</s:if>
	       		<s:else>
	       			<input type="button" value="일본어월말시험등록" class="btn btn-primary disabled" id="inserttestdisablebtn"><br>
	       		</s:else>
       		</div><br>
       		<!-- 검색기능 -->
       		 <p style="text-align: center; font-size: 30px;"><b>[월말시험반별검색]</b><p>
